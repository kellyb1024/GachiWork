...
const Search = ({ onOpenBoard, onTab, onOpenPost }) => {
  ...
  const hashtags = [
    { tag: 'wages', count: '1.2k' },
    { tag: 'dorm', count: '842' },
    { tag: 'visa', count: '731' },
    { tag: 'overtime', count: '512' },
    { tag: 'safety', count: '438' },
    { tag: 'health', count: '402' },
    { tag: 'contract', count: '356' },
    { tag: 'severance', count: '289' },
    { tag: 'eps', count: '241' },
    { tag: 'translator', count: '198' },
  ];
    ...
    <Chrome active="search" onTab={onTab}>
      ...
      <div className="scroll pad-bottom">
        <div className="search-bar">
          <Icon.search size={16}/>
          <span>Search jobs, situations, topics...</span>
        </div>
        ...
        <div className="sec-title">Industry groups</div>
        ...
        <div className="sec-title">Trending hashtags</div>
        <div className="trend-tags">
          {hashtags.map((h, i) => (
            <button key={h.tag} className="trend-tag">
              <span className="tt-rank">#{i+1}</span>
              <span className="tt-label">#{h.tag}</span>
              <span className="tt-count">{h.count}</span>
            </button>
          ))}
        </div>
