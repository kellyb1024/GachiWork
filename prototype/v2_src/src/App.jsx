...
const App = () => {
  ...
  const openChat = () => go({ name: 'chat' });
  const openUser = (uid) => go({ name: 'user', uid });
  ...
  useEffect(() => {
    const handler = (ev) => {
      ...
      if (d.type === '__activate_edit_mode') setTweaksOpen(true);else
  ...
  // External navigation (used by PPTX capture)
  useEffect(() => {
    window.__goRoute = (r) => {setHistory([]);setRoute(r);};
  }, []);
  ...
  if (name === 'landing') {
    ...
    view = <Signup key={`signup-${route.step || 1}`} initialStep={route.step || 1} onBack={() => setRoute({ name: 'landing' })}
    onDone={() => {setHistory([]);setRoute({ name: 'home' });}} />;
  } else if (name === 'home') {
    ...
    view = <BoardsHub onTab={goTab} onOpenBoard={openBoard} />;
  } else if (name === 'inbox') {
    view = <Inbox onTab={goTab} onOpenRisk={openRisk} onOpenPost={openPost} onOpenChat={openChat} />;
  } else if (name === 'profile') {
    view = <Profile onTab={goTab} onOpenPost={openPost} onOpenUser={openUser} />;
  } else if (name === 'user') {
    view = <Profile onTab={goTab} onOpenPost={openPost} onBack={back} viewUserId={route.uid} />;
  } else if (name === 'compose') {
    view = <Composer onBack={back} onPublish={() => {setHistory([]);setRoute({ name: 'post', pid: 'p_me_dorm' });}}
    onOpenPost={openPost}
    layout={tweaks.composerLayout} />;
  } else if (name === 'post') {
    view = <PostDetail postId={route.pid} onBack={back}
    onOpenRisk={openRisk} onOpenExpert={openExpert} onOpenPost={openPost} onOpenUser={openUser}
    ...
    view = <Expert onBack={back} onOpenChat={openChat} />;
  } else if (name === 'chat') {
    view = <ChatRoom onBack={back} />;
  } else {
  ...
  return (
    <>
      <div className={`app-root bg-${tweaks.bgTone || 'tinted'}`}>{view}</div>
      {tweaksOpen &&
...
const TweakPanel = ({ tweaks, setTweak, onClose, onNav }) => {
  ...
  { key: 'evalStyle', label: 'Evaluation UX', values: [
    { v: 'buttons', l: 'Helpful pill' }, { v: 'stars', l: 'Stars' }, { v: 'emoji', l: 'Emoji reactions' }]
  },
  { key: 'composerLayout', label: 'Composer layout', values: [
    { v: 'single', l: 'Single screen' }, { v: 'step', l: 'Step by step' }]
  },
  { key: 'bgTone', label: 'Background tone', values: [
    { v: 'tinted', l: 'Cobalt tint' }, { v: 'white', l: 'Pure white' }, { v: 'gray', l: 'Soft gray' }]
  }];
      ...
      <div className="tweak-body">
        ...
        <div className="tweak-group">
          ...
          <div className="tweak-options">
            ...
            <button onClick={() => onNav({ name: 'signup', step: 1 })}>Signup · Visa</button>
            <button onClick={() => onNav({ name: 'signup', step: 2 })}>Signup · Username</button>
            <button onClick={() => onNav({ name: 'signup', step: 3 })}>Signup · Password</button>
            <button onClick={() => onNav({ name: 'signup', step: 4 })}>Signup · Nationality</button>
            <button onClick={() => onNav({ name: 'signup', step: 5 })}>Signup · Language</button>
            <button onClick={() => onNav({ name: 'home' })}>Home</button>
            <button onClick={() => onNav({ name: 'boards' })}>Explore</button>
            <button onClick={() => onNav({ name: 'inbox' })}>Inbox</button>
            <button onClick={() => onNav({ name: 'profile' })}>Profile</button>
            <button onClick={() => onNav({ name: 'user', uid: 'u_vn_187' })}>Profile (other)</button>
            <button onClick={() => onNav({ name: 'compose' })}>Compose</button>
            <button onClick={() => onNav({ name: 'post', pid: 'p_me_dorm' })}>Published (dorm)</button>
            ...
            <button onClick={() => onNav({ name: 'chat' })}>Chat</button>
            <button onClick={() => onNav({ name: 'board', bkey: 'manufacturing' })}>Group</button>
          </div>
        </div>
      </div>
    </div>);
